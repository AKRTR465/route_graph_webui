...
REGISTRY_PACKAGE = "route_graph_webui.resources"
REGISTRY_PATH = f"{REGISTRY_PACKAGE}/{REGISTRY_FILENAME}"
...
def load_registered_env_names(registry_path: str | Path | None = None) -> tuple[list[str], str]:
    if registry_path:
        ...
        resource = resources.files(REGISTRY_PACKAGE).joinpath(REGISTRY_FILENAME)
        registry_label = str(resource)
    ...
    try:
        ...
        raise EnvRegistryError(f"Environment registry file not found: {registry_label}") from exc
    except json.JSONDecodeError as exc:
        raise EnvRegistryError(f"Environment registry file is not valid JSON: {registry_label}") from exc

    if not isinstance(data, dict):
        raise EnvRegistryError(f"Environment registry must be a JSON object: {registry_label}")
    ...
    if ENV_NAME_KEY in data:
        ...
        raise EnvRegistryError(f"Environment registry must contain `{ENV_NAME_KEY}` in {registry_label}")

    if not isinstance(raw_env_names, list):
        raise EnvRegistryError(f"`{ENV_NAME_KEY}` must be a JSON array in {registry_label}")

    normalized: list[str] = []
    seen: set[str] = set()
    for index, raw_value in enumerate(raw_env_names):
        if not isinstance(raw_value, str):
            raise EnvRegistryError(f"`{ENV_NAME_KEY}[{index}]` must be a string in {registry_label}")

        try:
            env_name = extract_env_name(raw_value)
        except ValueError as exc:
            raise EnvRegistryError(
                f"`{ENV_NAME_KEY}[{index}]` is invalid in {registry_label}: {exc}"
            ) from exc

        if env_name in seen:
            raise EnvRegistryError(f"Duplicate env_name `{env_name}` found in {registry_label}")

        seen.add(env_name)
        normalized.append(env_name)

    return normalized, registry_label
