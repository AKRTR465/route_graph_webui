...
from route_graph_webui.runtime_support.bootstrap_paths import inject_project_root as _inject_project_root
from route_graph_webui.runtime_support.bootstrap_paths import resolve_project_path as _resolve_project_path
from route_graph_webui.runtime_support.env_registry import (
    EnvRegistryError,
    build_env_id,
    extract_env_name,
...
from route_graph_webui.storage.graph_store import (
    DATA_DIR,
...
def ensure_project_root(start_path: str | Path | None = None) -> Path:
    """Inject the external UAV-MeM project root only when runtime code needs it."""
    global PROJECT_ROOT
    PROJECT_ROOT = _inject_project_root(start_path if start_path is not None else __file__)
    return PROJECT_ROOT


def inject_project_root(start_path: str | Path | None = None) -> Path:
    return ensure_project_root(start_path)
